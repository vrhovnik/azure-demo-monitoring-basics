...
namespace Monitoring.Basic.Services;

public class CleanBingNewsService : INewsService
{
    private readonly HttpClient client;
    private readonly ILogger<CleanBingNewsService> logger;
    private readonly BingServiceSettings bingServiceOptions;
    private readonly SqlSettings sqlSettings;

    public CleanBingNewsService(HttpClient client,
        IOptions<BingServiceSettings> bingServiceSettingsValue,
        IOptions<SqlSettings> sqlSettingsValue,
        ILogger<CleanBingNewsService> logger)
    {
        this.client = client;
        this.logger = logger;
        bingServiceOptions = bingServiceSettingsValue.Value;
        sqlSettings = sqlSettingsValue.Value;
    }
    ...
    public async Task<bool> SaveToFavoritesAsync(NewsModel model)
    {
        ...
        await using var connection = new SqlConnection(sqlSettings.ConnectionString);
